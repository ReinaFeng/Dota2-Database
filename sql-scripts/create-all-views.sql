...
SELECT 
  hero.name, item.name, it_times.times
FROM(
  SELECT 
  ...
  FROM(
    ...
    FROM 
      user_item, player
    WHERE 
      player.steam_id=user_item.steam_id AND player.match_id=user_item.match_id) AS hero_item
    GROUP BY 
      hero_item.item_id) AS it_times, hero, item
WHERE 
